...
public class BookDao {
	...
	public List<BookVo> selectBooksBySearch(BookVo bookVo) {
		...
		try {
			...
			}, "%" + bookVo.getB_name() + "%");} catch (Exception e) {
				e.printStackTrace();
			}

		return bookVos != null && bookVos.size() > 0 ? bookVos : null;
	}
		...
		try {
			bookVos = jdbcTemplate.query(sql, new RowMapper<BookVo>() {
				...
				public BookVo mapRow(ResultSet rs, int rowNum) throws SQLException {
					...
					bookVo.setB_rantal_able(rs.getInt("b_rantal_able"));
					bookVo.setB_reg_date(rs.getString("b_reg_date"));
					bookVo.setB_mod_date(rs.getString("b_mod_date"));return bookVo;
				}
			}, b_no);
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {
			rentalBookVos = jdbcTemplate.query(sql, new RowMapper<RentalBookVo>() {
				...
				public RentalBookVo mapRow(ResultSet rs, int rowNum) throws SQLException {
					...
					rentalBookVo.setRb_mod_date(rs.getString("rb_mod_date"));rentalBookVo.setB_thumbnail(rs.getString("b_thumbnail"));
					...
					rentalBookVo.setU_m_mod_date(rs.getString("u_m_mod_date"));return rentalBookVo;
				}
			});
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<HopeBookVo> selectHopeBooks() {
		...
		String sql = "SELECT * FROM tbl_hope_book hb " + "JOIN tbl_user_member um " + "ON hb.u_m_no ="
				+ " um.u_m_no " + "ORDER BY hb.hb_no DESC";
		...
		try {
			hopeBookVos = jdbcTemplate.query(sql, new RowMapper<HopeBookVo>() {
				...
				public HopeBookVo mapRow(ResultSet rs, int rowNum) throws SQLException {
					...
					hopeBookVo.setHb_result_last_date(rs.getString("hb_result_last_date"));hopeBookVo.setU_m_no(rs.getInt("u_m_no"));
